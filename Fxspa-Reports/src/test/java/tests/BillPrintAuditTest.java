...
public class BillPrintAuditTest extends BaseTest {
    ...
    public void verifyBillPrintAuditReportNavigation() {
        ...
        reportSteps.navigateToSpaReport(ModuleName.BILL_PRINT_AUDIT_REPORT);
        reportPage.clickOutlets();
        reportSteps.configureFiltersAndGenerate();

        AssertionUtils.assertTrueWithMessage(
            reportPage.isBillPrintAuditReportGenerated(),
            "Bill Print Audit Report generated successfully",
            "Failed to generate Bill Print Audit Report"
        );
    }

    @Test(priority = 2, dependsOnMethods = {"verifyBillPrintAuditReportNavigation"})
