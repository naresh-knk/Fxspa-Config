...
import com.idsnext.pages.StaffUtilizationPage;
...
public class StaffUtilizationTest extends BaseTest {

        private final String DOWNLOAD_PATH = "C:\\Users\\Deepshika\\Downloads";

    @Test
    public void verifyStaffUtilizationNavigation() {
        ReportNavigationSteps reportSteps = new ReportNavigationSteps(driver);
        StaffUtilizationPage reportPage = new StaffUtilizationPage(driver);

        reportSteps.navigateToSpaReport(ModuleName.STAFF_UTILIZATION);
        reportPage.clickOutlets();
        reportSteps.configureFiltersAndGenerate();

        AssertionUtils.assertTrueWithMessage(
            reportPage.isStaffUtilizationGenerated(),
            "Staff Utilization Report generated successfully",
            "Failed to generate Staff Utlization Report"
        );
    }

     @Test(priority = 2, dependsOnMethods = {"verifyStaffUtilizationNavigation"})
