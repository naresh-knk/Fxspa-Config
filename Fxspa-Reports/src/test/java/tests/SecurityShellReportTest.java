...
import com.idsnext.pages.SecurityShellReportPage;
...
public class SecurityShellReportTest extends BaseTest {

        private final String DOWNLOAD_PATH = "C:\\Users\\Deepshika\\Downloads";

    @Test
    public void verifySecurityShellReportNavigation() {
        ReportNavigationSteps reportSteps = new ReportNavigationSteps(driver);
        SecurityShellReportPage reportPage = new SecurityShellReportPage(driver);

        reportSteps.navigateToSpaReport(ModuleName.SECURITY_SHELL_REPORT);
        reportSteps.generate();

        AssertionUtils.assertTrueWithMessage(
            reportPage.isSecurityShellReportGenerated(),
            "Security Shell Report generated successfully",
            "Failed to generate Security Shell Report"
        );
    }

     @Test(priority = 2, dependsOnMethods = {"verifySecurityShellReportNavigation"})
