...
import com.idsnext.pages.SettlementReportPage;
...
public class SettlementReportTest extends BaseTest {

        private final String DOWNLOAD_PATH = "C:\\Users\\Deepshika\\Downloads";

    @Test
    public void verifySettlementReportNavigation() {
        ReportNavigationSteps reportSteps = new ReportNavigationSteps(driver);
        SettlementReportPage reportPage = new SettlementReportPage(driver);

        reportSteps.navigateToSpaReport(ModuleName.SETTLEMENT_REPORT);

        reportSteps.configureFiltersAndGenerate();

        AssertionUtils.assertTrueWithMessage(
            reportPage.isSettlementReportGenerated(),
            "Settlement Report generated successfully",
            "Failed to generate Settlement Report"
        );
    }

     @Test(priority = 2, dependsOnMethods = {"verifySettlementReportNavigation"})
