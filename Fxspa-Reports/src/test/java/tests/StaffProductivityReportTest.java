...
import com.idsnext.pages.StaffProductivityReportPage;
...
public class StaffProductivityReportTest extends BaseTest {

        private final String DOWNLOAD_PATH = "C:\\Users\\Deepshika\\Downloads";

    @Test
    public void verifyStaffProductivityReportNavigation() {
        ReportNavigationSteps reportSteps = new ReportNavigationSteps(driver);
        StaffProductivityReportPage reportPage = new StaffProductivityReportPage(driver);

        reportSteps.navigateToSpaReport(ModuleName.STAFF_PRODUCTIVITY);
        reportSteps.configureFiltersAndGenerate();

        AssertionUtils.assertTrueWithMessage(
            reportPage.isStaffProductivityReportGenerated(),
            "Staff Productivity Report generated successfully",
            "Failed to generate Staff Productivity Report"
        );
    }

     @Test(priority = 2, dependsOnMethods = {"verifyStaffProductivityReportNavigation"})
