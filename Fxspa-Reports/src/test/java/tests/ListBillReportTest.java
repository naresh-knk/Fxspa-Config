...
import com.idsnext.pages.ListBillReportPage;
...
public class ListBillReportTest extends BaseTest {

        private final String DOWNLOAD_PATH = "C:\\Users\\Deepshika\\Downloads";

    @Test
    public void verifyListBillReportNavigation() {
        ReportNavigationSteps reportSteps = new ReportNavigationSteps(driver);
        ListBillReportPage reportPage = new ListBillReportPage(driver);

        reportSteps.navigateToSpaReport(ModuleName.LIST_BILL_REPORT);

        reportSteps.configureFiltersAndGenerate();

        AssertionUtils.assertTrueWithMessage(
            reportPage.isListBillReportGenerated(),
            "List Bill Report generated successfully",
            "Failed to generate List Bill Report"
        );
    }

     @Test(priority = 2, dependsOnMethods = {"verifyListBillReportNavigation"})
