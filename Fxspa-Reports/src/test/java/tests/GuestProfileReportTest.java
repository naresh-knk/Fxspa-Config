...
import com.idsnext.pages.GuestProfileReportPage;
...
public class GuestProfileReportTest extends BaseTest {

        private final String DOWNLOAD_PATH = "C:\\Users\\Deepshika\\Downloads";

    @Test
    public void verifyGuestProfileReportNavigation() {
        ReportNavigationSteps reportSteps = new ReportNavigationSteps(driver);
        GuestProfileReportPage reportPage = new GuestProfileReportPage(driver);

        reportSteps.navigateToSpaReport(ModuleName.GUEST_PROFILE_REPORT);

        reportSteps.configureFiltersAndGenerate();

        AssertionUtils.assertTrueWithMessage(
            reportPage.isGuestProfileReportGenerated(),
            "Guest Profile Report generated successfully",
            "Failed to generate Guest Profile Report"
        );
    }

     @Test(priority = 2, dependsOnMethods = {"verifyGuestProfileReportNavigation"})
