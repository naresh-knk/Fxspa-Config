...
import com.idsnext.pages.SpaTaxRegisterPage;
...
public class SpaTaxRegisterTest extends BaseTest {

        private final String DOWNLOAD_PATH = "C:\\Users\\Deepshika\\Downloads";

    @Test
    public void verifySpaTaxRegisterNavigation() {
        ReportNavigationSteps reportSteps = new ReportNavigationSteps(driver);
        SpaTaxRegisterPage reportPage = new SpaTaxRegisterPage(driver);

        reportSteps.navigateToSpaReport(ModuleName.SPA_TAX_REGISTER);
        reportSteps.configureFiltersAndGenerate();

        AssertionUtils.assertTrueWithMessage(
            reportPage.isSpaTaxRegisterGenerated(),
            "Spa Tax Register generated successfully",
            "Failed to generate Spa Tax Register"
        );
    }

    // @Test(priority = 2, dependsOnMethods = {"verifySpaTaxRegisterNavigation"})
